...
public interface AddressDAL {
    ...
    @Update("update `Address` set name=#{name},phone=#{phone},province=#{province},city=#{city},area=#{area},detail=#{detail},poster=#{poster} " +
            "where id=#{id}")
    ...
    @Select("select id,name,phone,province,city,area,detail,poster,userId,shopId from `Address` where userId=#{userId} and shopId=#{shopId} limit 1")
    Address getTop(@Param("userId") String userId, @Param("shopId") String shopId);
